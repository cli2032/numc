...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    if (!PyObject_TypeCheck(args, &Matrix61cType)) {
    	PyErr_SetString(PyExc_TypeError, "ERROR: Argument must be of type numc.Matrix!");
        return NULL;
    }

    if (((Matrix61c *)args)->mat->rows != self->mat->rows || ((Matrix61c *)args)->mat->cols != self->mat->cols) {
    	PyErr_SetString(PyExc_TypeError, "ERROR: The given matrices do not all have the same valid dimensions!");
        return NULL;
    }
    ...
    Matrix61c* rv = (Matrix61c *) Matrix61c_new(&Matrix61cType, NULL, NULL);
    rv->mat = result;
    rv->shape = PyTuple_Pack(2, PyLong_FromLong(result->rows), PyLong_FromLong(result->cols));
    return (PyObject *)rv;
}
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
    ...
    if (self->mat->cols != ((Matrix61c *)args)->mat->rows) {
    	PyErr_SetString(PyExc_TypeError, "ERROR: The given matrices do not have valid dimensions for multiplication!");
        return NULL;
    }

    matrix *result;
    int alloc_failed = allocate_matrix(&result, self->mat->rows, ((Matrix61c *)args)->mat->cols);
    if (alloc_failed) {
    	PyErr_SetString(PyExc_RuntimeError, "ERROR: Matrix memory allocation failed!");
        return NULL;
    }
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    if (!PyLong_Check(pow) || (PyLong_Check(pow) && PyLong_AsLong(pow) < 1)) {
    	PyErr_SetString(PyExc_TypeError, "ERROR: Power must be a positive integer!");
        return NULL;
    }

    if (self->mat->cols != self->mat->rows) {
    	PyErr_SetString(PyExc_TypeError, "ERROR: The given matrix does not have valid dimensions to be squared!");
        return NULL;
    }
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
    ...
    if (PyArg_UnpackTuple(args, "args", 3, 3, &arg1, &arg2, &arg3)) {
        // arguments are (row, col, val)
        if (arg1 && arg2 && arg3 && PyLong_Check(arg1) && PyLong_Check(arg2) && PyFloat_Check(arg3)) {
        	if (PyLong_AsLong(arg1) >= self->mat->rows || PyLong_AsLong(arg2) >= self->mat->cols ||
        		PyLong_AsLong(arg1) < 0 || PyLong_AsLong(arg2) < 0) {
        		PyErr_SetString(PyExc_IndexError, "ERROR: One or more arguments are out of bounds!");
            	return NULL;
        	} else {
        		set(self->mat, PyLong_AsLong(arg1), PyLong_AsLong(arg2), PyFloat_AsDouble(arg3));
            	Py_RETURN_NONE;
        	}
        } else {
            PyErr_SetString(PyExc_TypeError, "ERROR: One or more arguments are of an invalid type!");
            return NULL;
        }
    } else {
        PyErr_SetString(PyExc_TypeError, "ERROR: Invalid number of arguments!");
        return NULL;
    }
}
    ...
    if (PyArg_UnpackTuple(args, "args", 2, 2, &arg1, &arg2)) {
        ...
        if (arg1 && arg2 && PyLong_Check(arg1) && PyLong_Check(arg2)) {
        	if (PyLong_AsLong(arg1) >= self->mat->rows || PyLong_AsLong(arg2) >= self->mat->cols ||
        		...
        		PyObject *rv = PyFloat_FromDouble(get(self->mat, PyLong_AsLong(arg1), PyLong_AsLong(arg2)));
            	return rv;
        	}
        } else {
            PyErr_SetString(PyExc_TypeError, "ERROR: One or more arguments are of an invalid type!");
